...
- Command (at the time): legacy ingest; now use three-pass pipeline, e.g. `uv run python -m examples.medlit.scripts.pass1_extract --input-dir examples/medlit/pmc_xmls --output-dir pass1_bundles --llm-backend ollama --papers "PMC12770061.xml"` then pass2_dedup and pass3_build_bundle (see run-ingest.sh).
