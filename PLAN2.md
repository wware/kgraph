...
- **Load:** In the medlit pipeline (e.g. `examples/medlit/scripts/pass1_extract.py` or the pipeline module that uses embeddings), locate the call to `cached_embedding_generator.cache.load()` (or equivalent). Verify it runs in `main()` **before** the first use of embeddings (e.g. before the loop that calls `orchestrator.ingest_document` or `extract_entities_from_document`). If load is conditional (e.g. only when cache file path is set), confirm that path is always set when running full ingest (e.g. `output_dir / "embeddings_cache.json"`).
...
| Ingest | Medlit pipeline scripts (pass1_extract / pipeline modules; verify cache path and single generator; optional stats log). Legacy ingest.py removed (PLAN10). |
